...
void output(Kiwii::Screench uv, Kiwii_Experiment::Experiment1 a, uint8_t Key_F1) {
...
int main() {
	...
	while (true) {
		...
		//output
		{
			...
			if (Key_P) uv.set_screen((uv.get_square() - 1) - 2, 'P');
			else uv.set_screen((uv.get_square() - 1) - 2, '-');

			uv.insert_hex(uv.get_square() - 3, seed);
			uv.insert_hex(uv.get_square() - 11, next_seed);

			if (Key_F5) uv.set_screen((uv.get_square() - 1) - 19, 'S');
			else uv.set_screen((uv.get_square() - 1) - 19, 'H');

			uv.insert_hex((uv.get_square() - 1) - 20, a.hours);
			uv.insert_hex((uv.get_square() - 1) - 22, a.day);
			uv.insert_hex((uv.get_square() - 1) - 26, a.year);
